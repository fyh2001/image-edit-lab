...
def caption_sample(sample, provider, rng, counts, weights=None, max_retries=3, images=None):
    """给单个 sample dict 生成一条 caption（就地写回 sample）。返回 (caption, style, ok)。

    counts: {style: 已产条数}，本函数会在成功后 +1（供跨样本亏空均衡）。
    """
    ...
    for _ in range(max(1, int(max_retries))):
        cap = provider.caption(f, style, images=images, rng=rng)
    ...
    return caption, style, ok


@register_task("vlm_caption")
def vlm_caption(payload):
    """payload: {raw_dir, provider?='stub', provider_params?, style_weights?, seed?=0,
                 max_retries?=3, use_images?=False}
    遍历 raw_dir/*/sample.json，逐个改写并落盘。返回 {raw_dir, n, verified, fallback, by_style}。
    """
    ...
    for sj in sorted(glob.glob(os.path.join(raw_dir, "*", "sample.json"))):
        ...
        _cap, style, ok = caption_sample(sample, provider, rng, counts,
                                         weights=weights, max_retries=max_retries, images=images)
    ...
    print(f"[vlm_caption] {raw_dir} | {n} 样本，校验通过 {verified}，回退 {fallback} | 风格 {by_style}")
    return {"raw_dir": raw_dir, "n": n, "verified": verified, "fallback": fallback, "by_style": by_style}
